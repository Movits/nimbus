...
function Resolve-VersionModal {
    ...
    for ($i = 1; $i -le $Retries; $i++)
    {
        ...
        $btn = Find-UiElement -Root $root `
            ...
            -ControlTypes @('ControlType.Button', 'ControlType.Hyperlink', 'ControlType.ListItem', 'ControlType.Text')
...
$handle = [NimbusChrome]::FindChromeWindow('Nuvem')
if ($handle -eq [IntPtr]::Zero) { $handle = [NimbusChrome]::FindChromeWindow('Personalizar') }
...
$menu = Find-UiElement -Root $root -Names @('Loja online') -NormalizedNames @('loja online') -ControlTypes @('ControlType.Button','ControlType.Hyperlink','ControlType.Text','ControlType.ListItem')
if ($null -ne $menu)
{
    Click-Element -Element $menu
    Start-Sleep -Milliseconds (2 * $WaitMs)
    $root = [System.Windows.Automation.AutomationElement]::FromHandle($handle)
    $layout = Find-UiElement -Root $root -Names @('Editar layout atual', 'Editar layout', 'Layout atual', 'Personalizar seu layout') -NormalizedNames @('editar layout atual', 'editar layout', 'layout atual', 'personalizar seu layout') -ControlTypes @('ControlType.Button','ControlType.Hyperlink','ControlType.Text','ControlType.ListItem')
    if ($null -ne $layout)
    {
...
if ($null -eq $layout)
{
    $layout = Find-UiElement -Root $root -Names @('Personalizar seu layout') -NormalizedNames @('personalizar seu layout') -ControlTypes @('ControlType.TabItem','ControlType.ListItem')
    if ($null -eq $layout)
    {
        Write-Output 'editlayout_not_found'
        throw 'Não achou opção de layout da Nuvemshop.'
    }
    Write-Output ('fallback_layout=' + $layout.Current.Name + ' / ' + $layout.Current.ControlType.ProgrammaticName)
    Click-Element -Element $layout
    Start-Sleep -Milliseconds (2 * $WaitMs)
    $root = [System.Windows.Automation.AutomationElement]::FromHandle($handle)
}
    $cssLink = Find-UiElement -Root $root `
        ...
        -ControlTypes @('ControlType.Button','ControlType.Hyperlink','ControlType.Text','ControlType.ListItem','ControlType.TabItem')
if ($null -eq $cssLink)
{
    $cssLink = Find-UiElement -Root $root `
        -Names @('css') `
...
$root = [System.Windows.Automation.AutomationElement]::FromHandle($handle)
$pubBtn = Find-UiElement -Root $root -Names @('Publicar alterações', 'Publicar') -NormalizedNames @('publicar alteracoes', 'publicar') -ControlTypes @('ControlType.Button','ControlType.Hyperlink')
