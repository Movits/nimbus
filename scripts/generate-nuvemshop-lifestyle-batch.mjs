...
function buildPrompt(task) {
  ...
  return [
    ...
    `REFERENCE 3 is the exact live YouDraw/Nuvemshop product mockup and is the authority for print side and print position. It is a FLAT garment, so do not judge print SIZE against it by eye.`,
    references,
    `Reproduce the sold product exactly. Apply the artwork unchanged and keep the same relative graphic bounding box and placement shown in REFERENCE 3. Match that relative size in both directions: a print smaller than REFERENCE 3 is as defective as a larger one, and must not be shrunk as a safety margin. REFERENCE 4, when present, defines artwork content only and must never override the print scale from REFERENCE 3. Do not enlarge, redraw, crop, simplify, replace lettering, invent symbols, alter signatures or add elements.`,
    pose,
